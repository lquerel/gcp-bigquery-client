...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data = std::fs::read("examples/data.json")
    ...
    let job_ref = create_bq_load_job(&client, &project_id, "test_batch_load", "test4", source_uri, &tmp_file_name).await?;

    while get_job_status(&client, &project_id, job_ref.job_id.as_ref().unwrap()).await?.state != Some("DONE".to_string()) {
...
pub async fn get_job_status(
    ...
    let job = client
        .job()
        .get_job(project_id, job_id, None)
        .await?;
...
pub async fn store_and_get_gcs_uri(gcs_bucket_name: &str, data: Vec<u8>, file_name: &str) -> Result<String, Box<dyn std::error::Error>> {
...
pub fn env_vars() -> (String, String) {
    let project_id = env::var("PROJECT_ID")
        .expect("Environment variable PROJECT_ID");
